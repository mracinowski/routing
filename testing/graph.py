...
def generate_graph(nodes, extra_edges):
    return generate_extra_edges(nodes, generate_spanning_tree(nodes), extra_edges)
...
def generate_network(
    ...
    for group in groups:
        ...
        gateways[group] = sample(
            nodes[group], randint(1, min(node_count, max_gateways_count))
        )
